...
RSpec.describe Tasker::Authorization::BaseCoordinator do
  ...
  before do
    ...
    Tasker::Configuration.configuration { |config| config.auth { |auth| auth.authorization_enabled = false } }
  end
  ...
  describe '#authorize!' do
    context 'when authorization is disabled' do
      before { Tasker::Configuration.configuration { |config| config.auth { |auth| auth.authorization_enabled = false } } }
    ...
    context 'when authorization is enabled' do
      before { Tasker::Configuration.configuration { |config| config.auth { |auth| auth.authorization_enabled = true } } }
