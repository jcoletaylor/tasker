...
RSpec.configure do |config|
  ...
  # to ensure that it's configured to serve OpenAPI documents from the same folder
  config.openapi_root = Rails.root.join('swagger').to_s

  # Define one or more OpenAPI documents and provide global metadata for each one
  # When you run the 'rswag:specs:swaggerize' rake task, the complete OpenAPI document will
