  ...
  class ViableStepDiscovery
    ...
    class << self
      ...
      def subscribe_to_orchestration_events(bus = nil)
        ...
        Rails.logger.info('ViableStepDiscovery subscribed to orchestration events')
      end
    end
    ...
    def discover_viable_steps_for_task(task_id)
      ...
      if viable_steps.any?
        ...
        Rails.logger.debug { "ViableStepDiscovery: Found #{viable_steps.size} viable steps: #{step_names.join(', ')}" }
